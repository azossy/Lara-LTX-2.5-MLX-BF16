...
| P3 | Port Gemma conditioning, scheduler, CFG/STG/res_2s, spatial upscaler, audio VAE/vocoder and single in-process media pipeline | Fixed prompt produces a playable MLX MP4 with audio | Native Gemma 4 conditioning, scheduler/guidance/res_2s, the 72-weight x2 upscaler, and the 58-tensor Audio VAE decoder path are implemented and pass CUDA boundaries. Vocoder/BWE, 48-block orchestration, two-stage wiring and media pipeline remain |
