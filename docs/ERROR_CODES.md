...
| LARA-QUALITY-001 | `LARA-QUALITY-001` | P5 corpus, generation, comparison or blind-review kit is invalid | Restore the versioned corpus/configuration, verify both media directories and rerun | `tools/quality` |
| LARA-QUALITY-002 | `LARA-QUALITY-002` | Resumable evaluator-model shard download, byte-size check or SHA-256 verification failed | Verify the TSV manifest, data-disk capacity, `aria2c`/`sha256sum`, network access and retry the supervisor; completed chunks are preserved | `tools/quality/autodl_aria2_supervisor.sh` |
| LARA-RELEASE-001 | `LARA-RELEASE-001` | GitHub/Hugging Face release identity, version, upstream pin, links or license bundle is inconsistent | Align `configs/release.toml`, package metadata and Hugging Face descriptor before publishing | `tools/release/validate_release_bundle.py` |
| LARA-PIPELINE-004 | `LARA-PIPELINE-004` | Lightweight release repository manifest is unreadable or declares an invalid schema/source | Restore `lara_ltx_model.toml` from the matching tagged release | `lara_ltx.pipeline.configuration` |
