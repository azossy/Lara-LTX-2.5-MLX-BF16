...
| P4/P5 | Replay the expanded stochastic CUDA trace, then run the multi-prompt audiovisual quality corpus | Frozen final-latent tolerances, documented differences and acceptance report | CUDA capture complete. The metadata-driven 1000x AV cross-timestep fix makes every exact input-preprocessor field pass. A CUDA-output-injected sampler replay exposed and fixed the incorrectly disabled official anchor refinement; all 31+7 sampler inputs now pass with worst NRMSE `1.14e-5`. The v6 exact-input Attention trace proves CUDA-aligned FP32 Q/K RMSNorm and RoPE accumulation; all 176 sub-operation comparisons pass. The corrected full replay improves final video NRMSE from `0.3555` to `0.1725`; audio is `0.1184` versus the prior `0.1095`, so the strict final-latent gate remains open. Two MLX corpus cases and paired diagnostics are complete. The 512x512/33-frame case caused heavy swap growth and OS OOM after about 70 minutes on M5 Max 128 GB, so full quality acceptance remains open. |
