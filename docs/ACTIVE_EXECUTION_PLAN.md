...
| P4/P5 | Replay the expanded stochastic CUDA trace, then run the multi-prompt audiovisual quality corpus | Frozen final-latent tolerances, documented differences and acceptance report | CUDA capture complete. The metadata-driven 1000x AV cross-timestep fix makes every exact input-preprocessor field pass; exact-input block-47 and output-head boundaries pass, while the strict full-trajectory final-latent gate still fails after cross-backend BF16 accumulation. Two MLX corpus cases and paired diagnostics are complete. The 512x512/33-frame case caused heavy swap growth and OS OOM after about 70 minutes on M5 Max 128 GB, so full quality acceptance remains open. |
