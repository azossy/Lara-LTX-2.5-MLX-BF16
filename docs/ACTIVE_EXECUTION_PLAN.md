...
| 5 | Capture core CUDA reference artifacts | Serialized conditioning, sampler, latent, decoder and audio boundary tensors | Complete: expanded v3 SDE/denoiser trace, v4 seven-block/three-pass deep trace and v5 block-0 internal trace are local and SHA-256 verified |
...
| P4/P5 | Replay the expanded stochastic CUDA trace, then run the multi-prompt audiovisual quality corpus | Frozen final-latent tolerances, documented differences and acceptance report | CUDA capture complete. The metadata-driven 1000x AV cross-timestep fix makes every exact input-preprocessor field pass. A CUDA-output-injected sampler replay exposed and fixed the incorrectly disabled official anchor refinement; all 31+7 sampler inputs now pass with worst NRMSE `1.14e-5`. The corrected full replay improves final video/audio NRMSE from `0.6856`/`0.8152` to `0.3555`/`0.1095`, but the strict final-latent gate remains open. Two MLX corpus cases and paired diagnostics are complete. The 512x512/33-frame case caused heavy swap growth and OS OOM after about 70 minutes on M5 Max 128 GB, so full quality acceptance remains open. |
| P6 | Measure and optimize on the target Apple Silicon hardware | Peak memory, latency and Metal utilization reports | Reduced-smoke repeat gate passes with zero released-memory growth. A measured 512x512/33-frame OOM now defines a concrete memory/performance optimization boundary; per-stage timing/utilization and canonical workload remain. |
